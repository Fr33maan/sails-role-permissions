...
exports.default = function (req, config) {
  ...
  if (askedRole === 'guest') {
    if (reqRole === 'guest') {
      ...
      throw new Error(_errorMessages.controllerNotAllowed + 'asked role is guest and req is ' + reqRole); //Deny
    }
  }
  ...
  if (_roleUtil2.default.isRoleAllowed(reqRole, askedRole, config.roles)) {
    ...
    throw new Error(_errorMessages.controllerNotAllowed + 'asked role is above user permission'); // Deny
  }
